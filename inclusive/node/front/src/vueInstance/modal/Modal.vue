      ...
      <div class="modal-box" v-show='show' id='my_modal_box'>
        <transition name='scale'>
          <div class='modal-body' v-show='show'>
            ...
            <div class='operate'>
                <Button class='btn-border-red margin-right' @on-click="choose(false,1)">{{cancelButtonText}}</Button>
                <Button class="jq-btn-red" @on-click="choose(true,1)">{{confirmButtonText}}</Button>
            </div>        
          </div>
        </transition>
      </div>
  ...
  export default{
    ...
    methods: {
      ...
      showModal(opts){
        ...
        let {show,type,title,subtitle,cancelButtonText,confirmButtonText,callback,close} = opts
        if(!title){
